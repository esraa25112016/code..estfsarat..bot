...
async def get_phone(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for i, record in enumerate(records, start=2):  # نبدأ من الصف الثاني (بعد العناوين)
        # تعديل المقارنة لضمان تطابق الرقم حتى لو كان محفوظ كـ float
        if str(record.get("telegram_id")).strip() == str(int(user_id)):
            sheet.update(f"A{i}", name)
            sheet.update(f"B{i}", phone)
            sheet.update(f"D{i}", username)
            sheet.update(f"E{i}", timestamp)
            updated = True
            break
